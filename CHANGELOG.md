...
producing pull requests a reviewer could not adopt — which is what let this release also carry the
first re-fit in a while.

### Changed

- **The heuristic's weights are re-fit against current arena data.** It only speaks for cards the
  win-rate feed does not cover, so most picks are unaffected; where it does speak, ~15% of same-class
  three-card choices change — well inside what resampling the same data moves (~30%), and the fit is
  reproducible on two machines. Worth knowing what did NOT cause it: the ~200 extra cards the newer
  HearthDb resolves move only 3% on their own. The rest is HSReplay's own 4-day window having turned
  over, which every weekly re-fit adopts anyway. Numbers in REPORT.md §18.
